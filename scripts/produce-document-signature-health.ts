...
async function main() {
  ...
  try {
    ...
    process.exitCode = result.result === "PUBLISHED" ? 0 : 1;
  } catch {
    console.log(JSON.stringify({ result: "OPERATIONAL_FAILURE", reason: "UNKNOWN_FAILURE", phase: "CONFIGURATION", durationMs: Math.max(0, performance.now() - started) }));
    process.exitCode = 1;
  }
}
void main();
