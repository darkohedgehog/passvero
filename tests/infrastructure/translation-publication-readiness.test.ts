...
for (const name of ["", " ", "a".repeat(201)]) test(`incomplete secondary name (${name.length}) blocks whole version`, async () => {
  const tx = { productTranslation: { findUnique: async()=>({productName:"Stolica"}), findMany: async()=>[{locale:"hr",productName:"Stolica"},{locale:"en",productName:name}] }, productDocument:{findMany:async()=>[]},productImage:{findFirst:async()=>null},productMaterial:{findMany:async()=>[]},productIdentifier:{findMany:async()=>[]} };
