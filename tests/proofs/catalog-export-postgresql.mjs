...
import { Pool } from 'pg';
const root=resolve('.'),work=mkdtempSync('/private/tmp/passvero-csv-proof-');
...
writeFileSync(work+'/prisma.config.ts',`export default {schema:${JSON.stringify(work+'/schema.prisma')},migrations:{path:${JSON.stringify(work+'/migrations')}},datasource:{url:process.env.CSV_PROOF_URL}};`);
const url=`postgresql://proof:local-proof@127.0.0.1:${port}/catalog_csv_test`;
...
try {
 ...
 admin=new Pool({connectionString:url.replace('/catalog_csv_test','/postgres')});assert.equal((await admin.query('SHOW data_directory')).rows[0].data_directory,data);
 await admin.query('CREATE DATABASE catalog_csv_test');
 run(process.execPath,[root+'/node_modules/prisma/build/index.js','migrate','deploy','--config',work+'/prisma.config.ts'],{CSV_PROOF_URL:url});
 console.log(run(process.execPath,['--import','tsx','--test','tests/integration/catalog-export-postgresql.test.ts'],{TEST_DATABASE_URL:url,CSV_PROOF_OUTPUT:work+'/measurements.json'}));
