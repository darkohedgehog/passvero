...
import { Pool } from 'pg';
const root=resolve('.'),work=mkdtempSync('/private/tmp/passvero-manufacturer-proof-');
...
const data=work+'/data',latest='20260917120000_add_manufacturer_snapshot';
...
cpSync('prisma/schema.prisma',work+'/schema.prisma');writeFileSync(work+'/prisma.config.ts',`export default {schema:${JSON.stringify(work+'/schema.prisma')},migrations:{path:${JSON.stringify(work+'/migrations')}},datasource:{url:process.env.MANUFACTURER_PROOF_URL}};`);
const url=(name,user='proof')=>`postgresql://${user}:local-proof@127.0.0.1:${port}/${name}`;
function deploy(name){run(process.execPath,[root+'/node_modules/prisma/build/index.js','migrate','deploy','--config',work+'/prisma.config.ts'],{MANUFACTURER_PROOF_URL:url(name)});}
...
try {
 ...
 for(const name of ['manufacturer_existing_test','manufacturer_empty_test'])await admin.query(`CREATE DATABASE ${name}`);
 deploy('manufacturer_existing_test');deploy('manufacturer_empty_test');
 const db=new Pool({connectionString:url('manufacturer_existing_test')});pools.push(db);
 ...
 const before=await snapshot();cpSync('prisma/migrations/'+latest,work+'/migrations/'+latest,{recursive:true});deploy('manufacturer_existing_test');deploy('manufacturer_empty_test');assert.deepEqual(await snapshot(),before);
 for(const table of ['EconomicOperator','ProductVersionManufacturer'])assert.equal((await db.query(`SELECT count(*)::int AS n FROM "${table}"`)).rows[0].n,0);
 await admin.query('CREATE ROLE manufacturer_test_app LOGIN NOSUPERUSER NOCREATEDB NOCREATEROLE');
 const empty=new Pool({connectionString:url('manufacturer_empty_test')});pools.push(empty);
 await empty.query('GRANT USAGE ON SCHEMA public TO manufacturer_test_app; GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA public TO manufacturer_test_app; REVOKE DELETE ON "EconomicOperator" FROM manufacturer_test_app');
 console.log('ADDITIVE_MIGRATION_EXISTING_AND_EMPTY=PASS; NO_BACKFILL; EXISTING_ROWS_UNCHANGED');
 console.log(run(process.execPath,['--import','tsx','--test','--test-concurrency=1','tests/integration/manufacturer-postgresql.test.ts','tests/integration/create-draft-from-published-postgresql.test.ts','tests/integration/publish-product-postgresql.test.ts'],{TEST_DATABASE_URL:url('manufacturer_empty_test','manufacturer_test_app')}));
