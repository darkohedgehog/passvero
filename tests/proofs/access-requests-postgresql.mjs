...
writeFileSync(work+'/prisma.config.ts',`export default {schema:${JSON.stringify(work+'/schema.prisma')},migrations:{path:${JSON.stringify(work+'/migrations')}},datasource:{url:process.env.ACCESS_PROOF_URL}};`);
const url=`postgresql://proof:local-proof@127.0.0.1:${port}/access_requests_test`;
