...
import { Pool } from 'pg';
const root=resolve('.'),work=mkdtempSync('/private/tmp/passvero-search-proof-');
...
writeFileSync(work+'/prisma.config.ts',`export default {schema:${JSON.stringify(work+'/schema.prisma')},migrations:{path:${JSON.stringify(work+'/migrations')}},datasource:{url:process.env.SEARCH_PROOF_URL}};`);
const url=`postgresql://proof:local-proof@127.0.0.1:${port}/catalog_search_test`;
...
try {
 ...
 admin=new Pool({connectionString:url.replace('/catalog_search_test','/postgres')});assert.equal((await admin.query('SHOW data_directory')).rows[0].data_directory,data);
 await admin.query('CREATE DATABASE catalog_search_test');
 run(process.execPath,[root+'/node_modules/prisma/build/index.js','migrate','deploy','--config',work+'/prisma.config.ts'],{SEARCH_PROOF_URL:url});
 console.log(run(process.execPath,['--import','tsx','--test','tests/integration/product-search-postgresql.test.ts'],{TEST_DATABASE_URL:url,SEARCH_PLAN_OUTPUT:work+'/plans.json'}));
