...
import { Pool } from 'pg';
const root=resolve('.'),work=mkdtempSync('/private/tmp/passvero-import-proof-');
...
writeFileSync(work+'/prisma.config.ts',`export default {schema:${JSON.stringify(work+'/schema.prisma')},migrations:{path:${JSON.stringify(work+'/migrations')}},datasource:{url:process.env.IMPORT_PROOF_URL}};`);
const url=`postgresql://proof:local-proof@127.0.0.1:${port}/catalog_import_test`;
...
try {
 ...
 admin=new Pool({connectionString:url.replace('/catalog_import_test','/postgres')});assert.equal((await admin.query('SHOW data_directory')).rows[0].data_directory,data);
 await admin.query('CREATE DATABASE catalog_import_test');
 run(process.execPath,[root+'/node_modules/prisma/build/index.js','migrate','deploy','--config',work+'/prisma.config.ts'],{IMPORT_PROOF_URL:url});
 console.log(run(process.execPath,['--import','tsx','--test','tests/integration/catalog-import-postgresql.test.ts'],{TEST_DATABASE_URL:url,IMPORT_PROOF_OUTPUT:work+'/measurements.json'}));
