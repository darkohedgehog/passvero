...
import { ProductActionIcon, editorDangerAction } from "./product-editor-ui";
...
export function ProductDocumentsSection({ productId, documents, evidence, canEdit, published }: Readonly<{
  ...
  return <section className="mt-6 min-w-0 rounded-xl border border-slate-200 bg-white p-4">
    ...
    {documents.length === 0 ? <p className="mt-3 rounded-lg border border-dashed border-slate-300 bg-slate-50 p-4 text-sm text-slate-600">{t("empty")}</p> : <ul className="mt-4 space-y-3">
      {documents.map(row => <li key={row.id} className="min-w-0 rounded-lg border border-slate-200 p-3">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          ...
          {editable ? <><button type="button" className={button} disabled={pending || editor !== null} onClick={() => { setError(null); setEditor(row); }}><ProductActionIcon name="edit" />{t("edit")}</button>
            <button type="button" className={editorDangerAction} disabled={pending || editor !== null} onClick={() => void remove(row)}><ProductActionIcon name="remove" />{t("remove")}</button></> : null}
        </div>
      </li>)}
    </ul>}
    {editable && editor === null ? <button type="button" className={`${button} mt-4`} disabled={pending} onClick={() => setEditor("NEW")}><ProductActionIcon name="add" />{t("add")}</button> : null}
...
export function AttachmentForm({ productId, evidence, row, onClose, onSuccess }: Readonly<{
  ...
  return <form onSubmit={submit} onChange={() => setDirty(true)} className="mt-5 min-w-0 rounded-lg bg-slate-50 p-4">
    <fieldset disabled={pending} className="min-w-0 space-y-4">
      ...
      <div className="flex flex-wrap gap-2"><button type="submit" className={button} disabled={uploadFailed}><ProductActionIcon name="upload" />{t(pending ? "pending" : uploaded ? "retry" : "save")}</button>
