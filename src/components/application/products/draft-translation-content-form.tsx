...
export function DraftTranslationContentForm(props: Readonly<{ productId: string; sourceLocale: string; initialValues: DraftTranslationContentValues; evidence: Record<"expectedDraftVersionId" | "expectedProductUpdatedAt" | "expectedDraftUpdatedAt" | "expectedSourceTranslationUpdatedAt", string>; detailHref: string; labels: Record<DraftTranslationContentField | "sourceLocale" | "save" | "saving" | "cancel" | "reload" | "validationError" | "staleWrite" | "draftNotEditable" | "forbidden" | "failure", string> }>) {
  ...
  return <form ref={form} onSubmit={submit} aria-busy={pending} className="space-y-6 rounded-xl border border-slate-200 bg-slate-50/70 p-4 sm:p-6" noValidate>
    ...
    <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end"><a href={props.detailHref} className="inline-flex min-h-11 max-w-full gap-2 whitespace-normal [overflow-wrap:anywhere] items-center justify-center rounded-lg border px-4 py-2.5 font-bold focus:ring-2 focus:ring-teal-600">{props.labels.cancel}</a><button type="submit" disabled={pending} aria-disabled={pending} className="inline-flex max-w-full items-center justify-center gap-2 whitespace-normal [overflow-wrap:anywhere] min-h-11 rounded-lg bg-teal-700 px-4 py-2.5 font-bold text-white focus:ring-2 focus:ring-teal-600 disabled:opacity-70"><ProductActionIcon name="save" />{pending ? props.labels.saving : props.labels.save}</button></div>
  </form>;
}
