...
export function CreateDraftFromPublishedAction({ data, labels }: Readonly<{ data: CreateDraftFromPublishedCommand; labels: CreateDraftLabels }>) {
  ...
  return <div aria-busy={pending} className="flex max-w-sm flex-col items-start gap-2">
    <button type="button" disabled={pending} onClick={create} className="inline-flex min-h-11 items-center justify-center rounded-lg border border-teal-700 px-4 py-2.5 text-sm font-bold text-teal-800 disabled:opacity-60 focus:outline-none focus:ring-2 focus:ring-teal-600">{pending ? labels.pending : labels.edit}</button>
